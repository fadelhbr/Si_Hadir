...
if (!isset($input["user_id"]) || !isset($input["unique_code"])) {
    sendResponse(
        "error",
        "Missing required parameters: user_id and unique_code"
    );
}
...
try {
    ...
    // Get employee data
    $stmt = $pdo->prepare(
        "SELECT id, status_aktif FROM pegawai WHERE user_id = ?"
    );
    $stmt->execute([$userId]);
    ...
    $attendance = getOrCreateAttendanceRecord(
        $pdo,
        $employeeId,
        $currentDate,
        $shiftSchedule["jadwal_shift_id"]
    );
    ...
    if (
        $attendance["waktu_masuk"] != "00:00:00" &&
    ...
    $shiftStart = new DateTime(
        $currentDate . " " . $shiftSchedule["jam_masuk"]
    );
    ...
    if ($attendance["waktu_masuk"] == "00:00:00") {
        ...
        if ($currentTime > $shiftEnd) {
            throw new Exception(
                "Melewati jam keluar shift dan tidak diperbolehkan absen"
            );
        }
        ...
        // Check if check-out time is before shift end
        $isEarlyLeave = $currentTime < $shiftEnd;

        // If early leave and no confirmation, request confirmation
        if ($isEarlyLeave && !$confirmEarlyLeave) {
            sendResponse(
                "confirm_needed",
                "Konfirmasi diperlukan untuk pulang lebih awal",
                [
                    "attendance_id" => $attendance["id"],
                    "early_leave" => true,
